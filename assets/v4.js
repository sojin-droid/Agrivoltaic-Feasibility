...
const FOOTNOTES = {
  ...
  2: '농림축산식품부, 『팜맵(농경지 전자지도)』, 2025년판 — 공공데이터포털 15104481–15104491, 항공·위성 판독(촬영 2019–2024 혼재).<span class="fn-note">용도: 실경작 판정 — 필지와 공간교차해 경작 면적 비율을 재고(전답과의 99.4% 실측), 30% 문턱으로 "실경작 확인" 층을 만든다.</span>',
  ...
  12: '본 연구 정본 데이터베이스(agrivoltaic_ledger_v1)와 도구(agv) — 원장·판정(elig_v2)·구역 태그(agpromo_tag)·간척 태그(reclaim_tag)·배제 사유(exclusion_reason)·연접·시나리오 테이블. 회귀 검사 전량 통과 상태의 산출 (2판, 2026-08-25).<span class="fn-note">용도: 이 사이트 모든 수치의 직접 출처 — 페이지는 이 DB의 export만 렌더한다. 자산별 구축 시점·원천 전량은 자료·한계 탭의 계보 표.</span>',
  ...
  14: '현행법 기준값(T14 상수) = 3,272,555필지 · 4,099,643,674.95㎡ — 2판 (ADR-0035, 2026-08-25 승인).<span class="fn-note">용도: 모든 비교의 고정 기준 — 데이터 생성 시 재조회와 정확 일치하지 않으면 산출이 중단된다. 개정 이력: 초판 → 배제 규정 원문 재검증 → 지목 복구 적용(2판). 용도지역 미상 67,875필지·30.53km²는 판정 보류로 별도 병기.</span>',
  ...
  17: '행정표준코드관리시스템(code.go.kr), 『법정동코드 전체자료』(2026-08-19 취득) 및 시군 경계 2023년판(신·구 행정코드 대조).<span class="fn-note">용도: 시군 이름·경계 표기 — 표기용 참조 자료로, 수치 산출에는 쓰지 않는다.</span>',
  18: '「농지법 시행규칙」 §32 — 2025-10-31판.<span class="fn-note">용도: 타인 소유 농지의 타용도 일시사용 신청에 소유자 사용승낙서 경로를 규정 — 간척지 임차 논의의 절차적 근거.</span>',
  19: 'Esri World Imagery — © Esri, Maxar, Earthstar Geographics.<span class="fn-note">용도: 지도·아틀라스의 위성 배경 — 표시 전용이며 수치 산출에는 쓰지 않는다. 아틀라스는 타일을 파일에 내장(오프라인 열람 가능).</span>',
