...
const FOOTNOTES = {
  ...
  14: '현행법 기준값(앵커 상수) = 5,719,511필지 · 5,582,552,382.14㎡ — 3판 (2026-08-27 승인, 실경작 조건 제거).<span class="fn-note">용도: 모든 비교의 고정 기준 — 데이터 생성 시 재조회와 정확 일치하지 않으면 산출이 중단됨.<br>개정 이력: 초판 → 배제 규정 원문 재검증 → 지목 복구 적용(2판) → 실경작 비율을 적격에서 제거(3판).<br>구 정의(경작 30% 이상) 값 3,272,555필지·4,099,643,674.95㎡는 폐지 — 회귀 검증용으로만 유지하며 정책 값으로 인용하지 않음.<br>용도지역 미상 필지는 판정 보류로 별도 병기.</span>',
  ...
  22: '한국전력공사, 『분산전원 연계정보』 온라인 조회 — 읍면동 단위 잔여 연계가능용량(배전선로 vol3, 표준 연계한도 반영). 전국 5,066 읍면동, 수집 2026-07-27–28(재조달 08-19), 원시 5,066파일·26,801레코드 보존.<span class="fn-note">용도: 산단 연계 탭의 계통 여유 채색 — 참고 지표이며 정본 판정에 불사용. 같은 설비의 값 상이는 최솟값으로 보수 확정, 균등배분(하한)–공유 미조정(상한) 병기, 원천 무응답 589곳은 알 수 없음으로 병기.</span>',
  23: '국토교통부 V-World 데이터 API — 산업단지 경계 LT_C_DAMDAN(산업입지정보시스템 등재 전국 1,363개: 일반 763·농공 483·국가 65·도시첨단 52, 2026-08-20 수집) · 읍면동 경계 LT_C_ADEMD_INFO(5,067개, 2026-08-26 수집).<span class="fn-note">용도: 산단 연계 탭의 경계 표기 — 표기용 참조 자료로, 수치 산출에는 쓰지 않음. 신·구 행정코드는 법정동코드 자료·필지 대표점으로 대조.</span>',
  24: '한국산업단지공단, 『전국등록공장현황』 등록공장현황자료(2024-12-31 기준) — 공공데이터포털 15105482, 2026-08-27 취득, 217,048사(산단 입주 84,111사).<span class="fn-note">용도: 산단 연계 탭의 입주기업명 검색 — 기업명→소재 산업단지 이동의 표기 참고용이며 정본 판정에 불사용. 단지명 대조는 별칭·유형 힌트 기준, 경계 미등재 단지 소재 기업은 검색 제외.</span>',
  ...
  footnotes() {
    ...
    sec.innerHTML = `<div class="wrap">
      ...
      FN_GROUPS.map(([g, nums]) =>
        ...
        `</ol>`).join('') +
      `<div class="fn">번호 [1]–[16]은 정책 브리프와 공통, [17]–[24]는 사이트 추가분.
