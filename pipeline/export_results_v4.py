...
df = con.execute("SELECT * FROM scenario_runs").fetch_df()
con.close()
...
for r in rows:
    ...
    if os.path.exists(sp):
        ...
        if (s.get('name') == r['name']
                and abs(s['params'].get('rho', -1) - (r.get('rho') or 0.3)) < 1e-9
...
out = {'generated': datetime.datetime.now().strftime('%Y-%m-%d %H:%M'),
       'note': '1판 (ADR-0034 승인 자산) — 값은 실경작 적용·장부면적 기준, MW는 참고(0.045kW/㎡)',
